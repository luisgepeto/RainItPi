...
tp_dict_key = "tp_dict_key"
tr_dict_key = "tr_dict_key"
...
tp_dict = { dir_key: file_util.get_test_pattern_root_path(),
                              ...
                              async_flag_key: False}
t_dict = { tp_dict_key: tp_dict, tr_dict_key : tr_dict, async_flag_key: False}
...
def get_test_callback(result):
    t_dict[async_flag_key] = False
    get_test_routine_callback(result["pattern_result_dict"])
    get_test_pattern_callback(result["routine_result_dict"])
    
def get_test_routine_callback(result):
    t_dict[tr_dict_key][async_flag_key] = False
    if result is None:
        auth_dict[refresh_key] = True
    else:
        if result[value_key] is not None and result[is_new_key]:
            t_dict[tr_dict_key][value_key] = result[value_key]
        if not out_dict[refresh_key]:
            out_dict[refresh_key] = result[is_new_key]

def get_test_pattern_callback(result):
    t_dict[tp_dict_key][async_flag_key] = False
    if result is None:
        auth_dict[refresh_key] = True
    else:
        if result[is_new_key]:
            t_dict[tp_dict_key][value_key] = result[value_key]
        if not out_dict[refresh_key]:
            out_dict[refresh_key] = result[is_new_key]        
...
def get_test_routine(old_value, token):
    try:
        ...
        resulting_dir = file_util.make_new_dir(t_dict[tr_dict_key][dir_key])
        file_util.add_timestamp_file(resulting_dir)
...
def get_test(old_pattern_value, old_routine_value, token):
    pattern_result_dict = get_test_pattern(old_pattern_value, token)
    routine_result_dict = get_test_routine(old_routine_value, token)
    return { "pattern_result_dict": pattern_result_dict, "routine_result_dict": routine_result_dict}
...
def get_test_pattern(old_value, token):
    try:
        ...
        resulting_dir = file_util.make_new_dir(t_dict[tp_dict_key][dir_key])
        file_util.add_timestamp_file(resulting_dir)
...
def output_test(clock_delay, latch_delay):
    test_pattern_timestamp = file_util.get_sampletimestamp_from(t_dict[tp_dict_key][dir_key])
    test_routine_timestamp = file_util.get_sampletimestamp_from(t_dict[tr_dict_key][dir_key])
    if test_routine_timestamp > test_pattern_timestamp and t_dict[tr_dict_key][value_key]:
        output_routine_list(t_dict[tr_dict_key][value_key], clock_delay, latch_delay)
    elif test_routine_timestamp < test_pattern_timestamp and t_dict[tp_dict_key][value_key] is not None:
        while True:
            hardware_manager.print_matrix(t_dict[tp_dict_key][value_key].matrix, clock_delay, latch_delay)        
...
def get_test_async():
    p = pool.Pool(1)
    p.apply_async(get_test,[t_dict[tp_dict_key][value_key], t_dict[tr_dict_key][value_key], auth_dict[value_key].token], callback=get_test_callback)
    p.close()
    return p
...
def display():
    ...
    if not out_dict[async_flag_key]:
        if t_dict[tp_dict_key][value_key] is not None or t_dict[tr_dict_key][value_key]:
...
def update_test():
    if (not file_util.is_dir_valid(t_dict[tp_dict_key][dir_key], s_dict[value_key].minutes_refresh_rate) or not file_util.is_dir_valid(t_dict[tr_dict_key][dir_key], s_dict[value_key].minutes_refresh_rate)) and not t_dict[async_flag_key] and is_auth_valid():
        t_dict[async_flag_key] = True
        get_test_async()
